...
import { CreateProductDto, ProductType, EligibilityRule } from '@/services/products';
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Select name="type" label="Product Type" value={productType} onChange={handleTypeChange} options={[{ label: 'Term Deposit', value: 'Term Deposit' }, { label: 'Loan', value: 'Loan' }]} />
            </div>
            <div className="mt-4">
        ...
        <div className="space-y-2">
          {(formData.interestRates || []).map((rate: any, index: number) => (
            <div key={index} className="flex items-center gap-2">
              <Input placeholder="Tenure (Months)" type="number" value={rate.tenure} onChange={e => handleRateChange(index, 'tenure', e.target.value)} />
              <Input placeholder="Rate (%)" type="number" value={rate.rate} onChange={e => handleRateChange(index, 'rate', e.target.value)} />
              <Button variant="danger" size="sm" onClick={() => removeRate(index)}><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
