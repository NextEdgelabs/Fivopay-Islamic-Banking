  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-stripe-text">Branch Performance Metrics</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h4 className="text-lg font-semibold text-stripe-text mb-4">Total Performance</h4>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Total Deposits:</span>
              <span className="text-sm font-medium text-green-600">
                {formatCurrency(filteredBranches.reduce((sum, branch) => sum + branch.totalDeposits, 0))}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Total Loans:</span>
              <span className="text-sm font-medium text-blue-600">
                {formatCurrency(filteredBranches.reduce((sum, branch) => sum + branch.totalLoans, 0))}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Total Customers:</span>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h4 className="text-lg font-semibold text-stripe-text mb-4">Average Metrics</h4>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Avg. Deposits/Branch:</span>
              <span className="text-sm font-medium text-green-600">
            ...
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Avg. Customers/Branch:</span>
              <span className="text-sm font-medium text-purple-600">
            ...
            <div className="flex justify-between">
              <span className="text-sm text-stripe-text-secondary">Avg. Employees/Branch:</span>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h4 className="text-lg font-semibold text-stripe-text mb-4">Top Performers</h4>
          <div className="space-y-3">
            {filteredBranches
              ...
              .map((branch, index) => (
                <div key={branch.id} className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-xs text-stripe-text-secondary">{branch.city}</div>
                  </div>
                  <span className="text-sm font-medium text-green-600">
      ...
      <div className="grid gap-6">
        {filteredBranches.map((branch) => (
          <div key={branch.id} className="bg-stripe-background-light p-6 rounded-lg">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h4 className="text-lg font-semibold text-stripe-text">{branch.branchName}</h4>
                <p className="text-sm text-stripe-text-secondary">{branch.city}, {branch.state}</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-4 rounded-lg">
                <p className="text-sm font-medium text-stripe-text-secondary">Customers</p>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 p-3 rounded-lg">
                <p className="text-sm font-medium text-stripe-text">Loan-to-Deposit Ratio</p>
                <p className="text-lg font-bold text-blue-600">
                  {((branch.totalLoans / branch.totalDeposits) * 100).toFixed(1)}%
                </p>
              </div>
              <div className="bg-green-50 p-3 rounded-lg">
                <p className="text-sm font-medium text-stripe-text">Deposits per Customer</p>
                <p className="text-lg font-bold text-green-600">
                  {formatCurrency(Math.round(branch.totalDeposits / branch.customerCount))}
                </p>
              </div>
              <div className="bg-purple-50 p-3 rounded-lg">
                <p className="text-sm font-medium text-stripe-text">Customer Growth Potential</p>
